...
struct ICUExport: AsyncParsableCommand {
  ...
  var outputHeadersDst: FilePath {
    outputDirectory.appending("SwiftFoundationICU").appending("icuSources").appending("include")
    ...
    func copyICUComponent(
      ...
      for item in componentContents {
        ...
        do {
          ...
          throw Error.fileManagerFailed(
            "cannot copy \(sourcePath) to \(destPath): \(e.localizedDescription)")
        }
      }
